...
export default function ContactForm(props: any) {
  ...
  return (
    <section className="flex flex-col lg:flex-row min-h-[85vh] border-b border-cream-200">
      ...
      <div className="w-full lg:w-5/12 flex flex-col justify-center px-8 sm:px-12 lg:px-20 py-24 bg-[#faf8f5] order-2 lg:order-1">
        ...
        <div className="space-y-10 flex flex-col text-xs tracking-[0.15em] uppercase font-semibold text-neutral-900">
          <div>
            ...
            <a href={`mailto:${email}`} className="hover:text-[#1f1e1c]/60 transition-colors border-b border-transparent hover:border-[#1f1e1c]/40 pb-1">
              {email}
            </a>
          </div>
          <div>
            <p className="text-[10px] text-neutral-500 mb-2 font-medium tracking-widest">Phone</p>
            <a href={`tel:${phone.replace(/\D/g,'')}`} className="hover:text-[#1f1e1c]/60 transition-colors">
              {phone}
            </a>
          </div>
          <div>
      ...
      {/* Right Column: form */}
      <div className="w-full lg:w-7/12 bg-[#faf8f5] flex flex-col justify-center px-8 sm:px-12 lg:px-24 py-24 lg:py-32 order-1 lg:order-2">
        ...
        {isSubmitted ? (
          <div className="bg-[#1f1e1c]/5 border border-[#1f1e1c]/20 p-12 text-center animate-fade-in flex flex-col items-center">
            ...
            <h4 className="font-serif text-3xl font-light text-[#1f1e1c] mb-4">Message Received</h4>
            ...
            <button 
              ...
              className="mt-8 text-[10px] tracking-[0.15em] uppercase text-[#1f1e1c] hover:text-[#333230] transition-colors border-b border-[#1f1e1c] pb-1 font-bold"
            >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col space-y-12 max-w-2xl">
            ...
            <div className="flex flex-col relative pt-4 group">
              <textarea name="message" id="message" required rows={5} value={formData.message} onChange={handleInputChange} className="peer w-full bg-transparent border-b border-neutral-400 py-3 text-sm text-[#1f1e1c] font-medium focus:outline-none focus:border-[#1f1e1c] transition-colors placeholder-transparent resize-none leading-relaxed" placeholder="Message" />
